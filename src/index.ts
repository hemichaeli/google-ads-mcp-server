...
function createMcpServer(): McpServer {
  ...
  server.registerTool("ads_create_bulk_rsa", {
    title: "Create Bulk Responsive Search Ads (RSA)",
    description: "Create multiple Responsive Search Ads (RSA) in a single API call (up to 100 ads). Each ad targets an ad group and provides its own headlines, descriptions, and final URLs.",
    inputSchema: {
      email: z.string(),
      customer_id: z.string(),
      ads: z.array(z.object({
        ad_group_id: z.string().describe("Ad group ID this RSA belongs to"),
        headlines: z.array(z.string()).min(3).max(15).describe("Headline texts (3-15)"),
        descriptions: z.array(z.string()).min(2).max(4).describe("Description texts (2-4)"),
        final_urls: z.array(z.string()).min(1).describe("Landing page URLs"),
        status: z.enum(["ENABLED", "PAUSED"]).default("ENABLED"),
      })).min(1).max(100).describe("Array of RSAs to create (max 100)"),
      login_customer_id: z.string().optional(),
    },
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async ({ email, customer_id, ads, login_customer_id }) => {
    const cid = customer_id.replace(/-/g, "");
    ...
    const r = await apiPost(email, `customers/${cid}/adGroupAds:mutate`, { operations }, lcid(email, login_customer_id));
    return { content: [{ type: "text", text: JSON.stringify({ requested: ads.length, result: r }, null, 2) }] };
  });

  server.registerTool("ads_get_network_performance", {
    title: "Get Network Performance",
    description: "Performance breakdown by ad network type (Google Search, Search Partners, Display Network, YouTube).",
    inputSchema: {
      email: z.string(), customer_id: z.string(),
      campaign_id: z.string().optional().describe("Optional: limit to one campaign"),
      date_range: z.enum(["LAST_7_DAYS","LAST_30_DAYS","THIS_MONTH","LAST_MONTH","LAST_90_DAYS"]).default("LAST_30_DAYS"),
      login_customer_id: z.string().optional(),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ email, customer_id, campaign_id, date_range, login_customer_id }) => {
    const cf = campaign_id ? `AND campaign.id = ${campaign_id}` : "";
    const r = await gaql(email, customer_id, `SELECT segments.ad_network_type, campaign.id, campaign.name, metrics.clicks, metrics.impressions, metrics.ctr, metrics.average_cpc, metrics.cost_micros, metrics.conversions, metrics.conversions_value FROM campaign WHERE segments.date DURING ${date_range} ${cf} ORDER BY metrics.cost_micros DESC`, lcid(email, login_customer_id));
    const rows = (r as Array<Record<string, any>>).map(x => ({
      network: x.segments?.adNetworkType, campaign: x.campaign?.name,
      clicks: x.metrics?.clicks, impressions: x.metrics?.impressions, ctr: pct(x.metrics?.ctr),
      avg_cpc: "$" + m2c(x.metrics?.averageCpc as number || 0), cost: "$" + m2c(x.metrics?.costMicros as number || 0),
      conversions: x.metrics?.conversions, conv_value: "$" + m2c(x.metrics?.conversionsValue as number || 0),
    }));
    return { content: [{ type: "text", text: JSON.stringify({ date_range, rows }, null, 2) }] };
  });
